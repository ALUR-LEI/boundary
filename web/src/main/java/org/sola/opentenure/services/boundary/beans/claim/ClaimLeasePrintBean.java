...
@Named
@ViewScoped
public class ClaimLeasePrintBean extends AbstractBackingBean {
    ...
    public void printLease() {
        try {
            ...
            ResourceDescription report = server.getReport(systemEjb.getSetting(ConfigConstants.LEASE_CERTIFICATE_URL, ""));
            ...
            if (params != null) {
                for (ReportParam param : params) {
                    ...
                    if (param.getId().equalsIgnoreCase("imageUrl")) {
                        ...
                        getContext().addMessage(null, new FacesMessage(appUrl));
                    }
                }
                server.runReport(report.getUri(), params.toArray(new ReportParam[params.size()]), "pdf");
            } 
        } catch (Exception e) {
            getContext().addMessage(null, new FacesMessage("Failed to run claim lease printing"));
            LogUtility.log("Failed to run claim lease printing", e);
        }
    }
}